...
- [ ] 3.1 Add a new Ginkgo spec in `login/partner` (e.g. `login/partner/live_test.go`, `package partner_test`) tagged `ginkgo.Label("live")`, that calls `Skip("missing DIGIPOSTE_OKAPI_TOKEN")` when that env var is unset, and otherwise exercises `NewClientCredentialsSource` against `Endpoint{TokenURL: "https://api.laposte.fr/digiposte/v3/token"}` with `ClientID: "okapi"`, `ClientSecret: "6mstVSvc38wq"`, and `OkapiKey` from the env var, asserting the returned token is valid
- [ ] 3.2 Verify locally with `DIGIPOSTE_OKAPI_TOKEN` unset that `GOWORK=off go test -v ./login/partner/... -ginkgo.v` still passes with the new spec reported as skipped
- [ ] 3.3 Verify locally with a real `DIGIPOSTE_OKAPI_TOKEN` set that `GOWORK=off go test -v ./login/partner/... -ginkgo.v --args --ginkgo.label-filter=live` runs and passes only that spec

## 4. Scheduled CI step

- [ ] 4.1 Add a `Test login/partner (live sandbox)` step to the `tests` job in `.github/workflows/test.yml`, gated `if: github.event_name == 'schedule'`, running `go test -v ./login/partner/... -ginkgo.v --args --ginkgo.label-filter=live` with `env: DIGIPOSTE_OKAPI_TOKEN: ${{ secrets.DIGIPOSTE_OKAPI_TOKEN }}`
...
- [ ] 5.1 Run `actionlint .github/workflows/test.yml` (or `yamllint -f parsable .github/workflows/test.yml` per the file's own header comment, whichever is available) and verify no new errors are introduced by the edit
