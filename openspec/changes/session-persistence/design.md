...
Simpler single code path ("obtain a session, then save it") beats special-casing "don't save if nothing changed." A `Save` on an unchanged, already-valid session is a harmless no-op for any reasonable `Store` implementation. A `Save` error is returned to the caller as a distinct, non-fatal problem (e.g. joined via `errors.Join` alongside a nil "login" error, or delivered through a callback) - the login itself must not fail just because persisting its result failed, per the spec.
