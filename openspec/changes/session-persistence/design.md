...
func New(store Store, newMethod NewMethod) login.Method
```
...
Simpler single code path ("obtain a session, then save it") beats special-casing "don't save if nothing changed." A `Save` on an unchanged, already-valid session is a harmless no-op for any reasonable `Store` implementation. A `Save` error must not fail an otherwise-successful `Login`: the decorator calls `store.Save` and discards its error, always returning the obtained token and cookies with a nil `Login` error. No separate reporting channel (callback, wrapped error, etc.) is added for this. The caller already owns `Store` and therefore already owns `Save`'s error - if it wants to observe, log, or retry a save failure, it does so inside its own `Save` implementation, the same way any other caller-supplied interface implementation is responsible for handling its own internal errors.

Alternative considered: an opt-in save-error callback/handler on `New`. Rejected - `Store` is a caller-supplied interface; the caller who writes `Save` already has direct access to any error it returns, so a second reporting path on top of that would be redundant indirection for a caller-owned concern.
...
- **[Risk]** A `Save` error is silently discarded by the decorator itself → **Mitigation**: acceptable because `Store` is caller-supplied; the caller's own `Save` implementation already has the error and is expected to handle it (log, retry, alert) internally. No SDK-level reporting channel is added on top of that.
